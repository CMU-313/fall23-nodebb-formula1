...
export const list = async function (req: GroupsRequest, res: Response) {
  ...
  const [groupsData, allowGroupCreation]: [GroupFullObject[], boolean] =
    await Promise.all([
  ...
  async function withOnineUserCount(
    groupData: GroupFullObject,
  ): Promise<GroupFullObject> {
    return {
      ...groupData,
      // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
      onlineUserCount: (await user.getGroupOnlineCount(
        groupData.name,
      )) as number,
    };
  }
...
export const details = async function (
  req: GroupsRequest,
  res: Response,
  next: NextFunction,
) {
  ...
  const groupName = (await groups.getGroupNameByGroupSlug(
    req.params.slug,
  )) as string;
  if (!groupName) {
    return next();
  ...
  // eslint-disable-next-line @typescript-eslint/no-unsafe-call
  groupData.onlineUserCount = (await user.getGroupOnlineCount(
    groupData.name,
  )) as number;
...
export const members = async function (
  req: GroupsRequest,
  res: Response & { query: { page: string } },
  next: NextFunction,
) {
