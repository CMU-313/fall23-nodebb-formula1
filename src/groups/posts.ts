...
interface GroupsInterface {
  onNewPostMade: (postData: {
    uid: number;
    cid: number;
    pid: number;
    timestamp: string;
  }) => Promise<void>;
  getLatestMemberPosts: (
    groupName: string,
    max: number,
    uid: string,
  ) => Promise<PostObject[]>;
...
export = function (Groups: GroupsInterface) {
  ...
  Groups.onNewPostMade = async function (postData) {
    const allgroupNames = await Groups.getUserGroupMembership(
  ...
  Groups.getLatestMemberPosts = async function (groupName, uid) {
    ...
    pids = (await privileges.posts.filter(
      "topics:read",
      pids,
      uid,
    )) as number[]; // fiter for privilege and access

    const allposts: PostObject[] = // Get all post summaries from pids
      // eslint-disable-next-line @typescript-eslint/no-unsafe-call
      (await posts.getPostSummaryByPids(pids, uid, {
        stripTags: false,
      })) as PostObject[];
    ...
    const assignedGroupNames = await Promise.all(
      ...
      mainPosts.map<Promise<TopicObject>>(
        // eslint-disable-next-line @typescript-eslint/no-unsafe-call
        async (post) =>
          (await topics.getTopicDataByPid(post.pid)) as Promise<TopicObject>,
      ),
    );
