...
module.exports = function (User: UserTemplate) {
  User.updateLastOnlineTime = async function (uid: string) {
    ...
    const userData: userDataTemplate = (await db.getObjectFields(
      `user:${uid}`,
      ["status", "lastonline"],
    )) as userDataTemplate;
    const now = Date.now();
  ...
  User.updateOnlineUsers = async function (uid: string) {
    ...
    const now = Date.now();
    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
    const userOnlineTime: string = (await db.sortedSetScore(
      "users:online",
      uid,
    )) as string;
    ...
    await plugins.hooks.fire("action:user.online", {
      uid: uid,
      timestamp: now,
    });
  };
  ...
  User.isOnline = async function (uid: string | string[]) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
    const lastonline: [] = (await db.sortedSetScores(
      "users:online",
      uid,
    )) as [];
   ...
   */
  User.getGroupOnlineCount = async function (
    groupName: string,
  ): Promise<number> {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unsafe-call
    const groupMemberMask = (await groups.isMembers(
      onlineUids,
      groupName,
    )) as boolean[];

    // Filter online user that are members in group
    const numOnlineInGroup = groupMemberMask.reduce(
      (count, isMember) => count + Number(isMember),
      0,
    );
