    ...
    Posts.getUserInfoForPosts = async function (uids, uid) {
        ...
        const result = await Promise.all(
            userData.map(async userData => {
                const [isMemberOfGroups, signature, customProfileInfo] = await Promise.all([checkGroupMembership(userData.uid, userData.groupTitleArray), parseSignature(userData, uid, uidsSignatureSet), plugins.hooks.fire('filter:posts.custom_profile_info', { profile: [], uid: userData.uid })]);
